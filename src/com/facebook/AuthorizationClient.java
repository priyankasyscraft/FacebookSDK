...
class AuthorizationClient implements Serializable {
	...
	// Constants for logging login-related data. Some of these are only used by
	// Session, but grouped here for
	// maintainability.
	...
	// Note: to ensure stability of column mappings across the four different
	// event types, we prepend a column
	// index to each name, and we log all columns with all events, even if they
	// are empty.
	...
	void setContext(final Activity activity) {
		...
		// If we are used in the context of an activity, we will always use that
		// activity to
	...
	void authorize(AuthorizationRequest request) {
		...
		if (request.needsNewTokenValidation() && !checkInternetPermission()) {
			// We're going to need INTERNET permission later and don't have it,
			// so fail early.
			return;
		}
	...
	void continueAuth() {
		if (pendingRequest == null || currentHandler == null) {
			throw new FacebookException(
					"Attempted to continue authorization without a pending request.");
		}
	...
	boolean onActivityResult(int requestCode, int resultCode, Intent data) {
		if (pendingRequest != null
				&& requestCode == pendingRequest.getRequestCode()) {
			return currentHandler.onActivityResult(requestCode, resultCode,
					data);
		}
		return false;
	}
	...
	boolean checkInternetPermission() {
		...
		if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
			String errorType = context
					.getString(R.string.com_facebook_internet_permission_error_title);
			String errorDescription = context
					.getString(R.string.com_facebook_internet_permission_error_message);
			complete(Result.createErrorResult(pendingRequest, errorType,
					errorDescription));
	...
	void tryNextHandler() {
		if (currentHandler != null) {
			logAuthorizationMethodComplete(currentHandler.getNameForLogging(),
					EVENT_PARAM_METHOD_RESULT_SKIPPED, null, null,
		...
		if (pendingRequest != null) {
			// We went through all handlers without successfully attempting an
			// auth.
			completeWithFailure();
		}
	}
	...
	private void completeWithFailure() {
		complete(Result.createErrorResult(pendingRequest,
				"Login attempt failed.", null));
	}
	...
	boolean tryCurrentHandler() {
		if (currentHandler.needsInternetPermission()
				&& !checkInternetPermission()) {
			addLoggingExtra(EVENT_EXTRAS_MISSING_INTERNET_PERMISSION,
					AppEventsConstants.EVENT_PARAM_VALUE_YES, false);
			return false;
		}
		...
		if (tried) {
			...
			addLoggingExtra(EVENT_EXTRAS_NOT_TRIED,
					currentHandler.getNameForLogging(), true);
		}
	...
	void completeAndValidate(Result outcome) {
		// Do we need to validate a successful result (as in the case of a
		// reauth)?
	...
	void complete(Result outcome) {
		// This might be null if, for some reason, none of the handlers were
		// successfully tried (in which case
		// we already logged that).
		if (currentHandler != null) {
			logAuthorizationMethodComplete(currentHandler.getNameForLogging(),
					outcome, currentHandler.methodLoggingExtras);
		}
	...
	StartActivityDelegate getStartActivityDelegate() {
		if (startActivityDelegate != null) {
			...
			return new StartActivityDelegate() {
				...
				@Override
				public Activity getActivityContext() {
					return pendingRequest.getStartActivityDelegate()
							.getActivityContext();
				}
			};
		}
	...
	RequestBatch createReauthValidationBatch(final Result pendingResult) {
		// We need to ensure that the token we got represents the same fbid as
		// the old one. We issue
		// a "me" request using the current token, a "me" request using the new
		// token, and a "me/permissions"
		...
		String validateSameFbidAsToken = pendingRequest
		...
		requestCurrentTokenPermissions.setCallback(new Request.Callback() {
			...
			public void onCompleted(Response response) {
				try {
					Session.PermissionsPair permissionsPair = Session
							.handlePermissionResponse(response);
					if (permissionsPair != null) {
						grantedPermissions.addAll(permissionsPair
								.getGrantedPermissions());
						declinedPermissions.addAll(permissionsPair
		...
		RequestBatch batch = new RequestBatch(requestCurrentTokenMe,
				requestNewTokenMe, requestCurrentTokenPermissions);
		...
		batch.addCallback(new RequestBatch.Callback() {
			...
			public void onBatchCompleted(RequestBatch batch) {
				try {
					Result result = null;
					if (fbids.size() == 2 && fbids.get(0) != null
							&& fbids.get(1) != null
						...
						AccessToken tokenWithPermissions = AccessToken
								.createFromTokenWithRefreshedPermissions(
										pendingResult.token,
										grantedPermissions, declinedPermissions);
						result = Result.createTokenResult(pendingRequest,
								tokenWithPermissions);
					} else {
						result = Result.createErrorResult(pendingRequest,
	...
	Request createGetPermissionsRequest(String accessToken) {
		...
		return new Request(null, "me/permissions", parameters, HttpMethod.GET,
				null);
	}
	...
	private AppEventsLogger getAppEventsLogger() {
		if (appEventsLogger == null
				|| !appEventsLogger.getApplicationId().equals(
	...
	private void logAuthorizationMethodStart(String method) {
		Bundle bundle = newAuthorizationLoggingBundle(pendingRequest
				.getAuthId());
		bundle.putLong(EVENT_PARAM_TIMESTAMP, System.currentTimeMillis());
		bundle.putString(EVENT_PARAM_METHOD, method);

		getAppEventsLogger().logSdkEvent(EVENT_NAME_LOGIN_METHOD_START, null,
				bundle);
	}

	private void logAuthorizationMethodComplete(String method, Result result,
			Map<String, String> loggingExtras) {
		logAuthorizationMethodComplete(method, result.code.getLoggingValue(),
				result.errorMessage, result.errorCode, loggingExtras);
	}

	private void logAuthorizationMethodComplete(String method, String result,
			String errorMessage, String errorCode,
			Map<String, String> loggingExtras) {
		Bundle bundle = null;
		if (pendingRequest == null) {
			// We don't expect this to happen, but if it does, log an event for
			// diagnostic purposes.
			bundle = newAuthorizationLoggingBundle("");
			bundle.putString(EVENT_PARAM_LOGIN_RESULT,
					Result.Code.ERROR.getLoggingValue());
		...
		getAppEventsLogger().logSdkEvent(EVENT_NAME_LOGIN_METHOD_COMPLETE,
				null, bundle);
	}

	static Bundle newAuthorizationLoggingBundle(String authLoggerId) {
		// We want to log all parameters for all events, to ensure stability of
		// columns across different event types.
	...
	class WebViewAuthHandler extends AuthHandler {
		...
		@Override
		boolean needsRestart() {
			// Because we are presenting WebView UI within the current context,
		...
		void cancel() {
			if (loginDialog != null) {
				// Since we are calling dismiss explicitly, we need to remove
				// the completion listener to prevent
		...
		boolean tryAuthorize(final AuthorizationRequest request) {
			...
			parameters.putString(ServerProtocol.DIALOG_PARAM_DEFAULT_AUDIENCE,
					audience.getNativeProtocolAudience());

			String previousToken = request.getPreviousAccessToken();
			if (!Utility.isNullOrEmpty(previousToken)
					&& (previousToken.equals(loadCookieToken()))) {
				parameters.putString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN,
						previousToken);
				// Don't log the actual access token, just its presence or
				// absence.
				addLoggingExtra(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN,
			...
			WebDialog.Builder builder = new AuthDialogBuilder(
					getStartActivityDelegate().getActivityContext(),
					applicationId, parameters).setE2E(e2e)
		...
		void onWebDialogComplete(AuthorizationRequest request, Bundle values,
			...
			if (values != null) {
				...
				AccessToken token = AccessToken.createFromWebBundle(
						request.getPermissions(), values,
				...
				if (error instanceof FacebookOperationCanceledException) {
					outcome = Result.createCancelResult(pendingRequest,
							"User canceled log in.");
				} else {
					// Something went wrong, don't log a completion event since
					// it will skew timing results.
					e2e = null;

					String errorCode = null;
					String errorMessage = error.getMessage();
					if (error instanceof FacebookServiceException) {
						FacebookRequestError requestError = ((FacebookServiceException) error)
								.getRequestError();
						errorCode = String.format("%d",
								requestError.getErrorCode());
						errorMessage = requestError.toString();
					}
					outcome = Result.createErrorResult(pendingRequest, null,
		...
		private void saveCookieToken(String token) {
			...
			context.getSharedPreferences(WEB_VIEW_AUTH_HANDLER_STORE,
					Context.MODE_PRIVATE).edit()
					.putString(WEB_VIEW_AUTH_HANDLER_TOKEN_KEY, token).apply();
		}

		private String loadCookieToken() {
			Context context = getStartActivityDelegate().getActivityContext();
			SharedPreferences sharedPreferences = context.getSharedPreferences(
					WEB_VIEW_AUTH_HANDLER_STORE, Context.MODE_PRIVATE);
	...
	class GetTokenAuthHandler extends AuthHandler {
		...
		@Override
		boolean needsRestart() {
			// if the getTokenClient is null, that means an orientation change
			// has occurred, and we need
			// to recreate the GetTokenClient, so return true to indicate we
			// need a restart
			return getTokenClient == null;
		}

		boolean tryAuthorize(final AuthorizationRequest request) {
			getTokenClient = new GetTokenClient(context,
		...
		void getTokenCompleted(AuthorizationRequest request, Bundle result) {
			...
			if (result != null) {
				ArrayList<String> currentPermissions = result
						.getStringArrayList(NativeProtocol.EXTRA_PERMISSIONS);
				List<String> permissions = request.getPermissions();
				if ((currentPermissions != null)
						&& ((permissions == null) || currentPermissions
								.containsAll(permissions))) {
					// We got all the permissions we needed, so we can complete
					// the auth now.
					AccessToken token = AccessToken.createFromNativeLogin(
							result,
							AccessTokenSource.FACEBOOK_APPLICATION_SERVICE);
					Result outcome = Result.createTokenResult(pendingRequest,
							token);
					completeAndValidate(outcome);
					return;
				}

				// We didn't get all the permissions we wanted, so update the
				// request with just the permissions
				// we still need.
				...
				if (!newPermissions.isEmpty()) {
					addLoggingExtra(EVENT_EXTRAS_NEW_PERMISSIONS,
	...
	abstract class KatanaAuthHandler extends AuthHandler {
		...
		protected boolean tryIntent(Intent intent, int requestCode) {
			...
			try {
				getStartActivityDelegate().startActivityForResult(intent,
						requestCode);
			} catch (ActivityNotFoundException e) {
				// We don't expect this to happen, since we've already validated
				// the intent and bailed out before
	...
	class KatanaProxyAuthHandler extends KatanaAuthHandler {
		...
		boolean tryAuthorize(AuthorizationRequest request) {
			...
			Intent intent = NativeProtocol.createProxyAuthIntent(context,
					request.getApplicationId(), request.getPermissions(), e2e,
					request.isRerequest(), request.getDefaultAudience());
		...
		boolean onActivityResult(int requestCode, int resultCode, Intent data) {
			...
			if (data == null) {
				...
				outcome = Result.createErrorResult(pendingRequest,
		...
		private Result handleResultOk(Intent data) {
			...
			String e2e = extras
					.getString(NativeProtocol.FACEBOOK_PROXY_AUTH_E2E_KEY);
			if (!Utility.isNullOrEmpty(e2e)) {
				logWebLoginCompleted(applicationId, e2e);
			}
			...
			if (error == null && errorCode == null && errorMessage == null) {
				AccessToken token = AccessToken.createFromWebBundle(
						pendingRequest.getPermissions(), extras,
				...
				return Result.createErrorResult(pendingRequest, error,
						errorMessage, errorCode);
			}
		}
	}
	...
	private void logWebLoginCompleted(String applicationId, String e2e) {
		...
		parameters.putLong(AnalyticsEvents.PARAMETER_WEB_LOGIN_SWITCHBACK_TIME,
				System.currentTimeMillis());
		parameters.putString(AnalyticsEvents.PARAMETER_APP_ID, applicationId);

		appEventsLogger.logSdkEvent(AnalyticsEvents.EVENT_WEB_LOGIN_COMPLETE,
				null, parameters);
	}
	...
	static class AuthDialogBuilder extends WebDialog.Builder {
		...
		public WebDialog build() {
			...
			parameters.putString(ServerProtocol.DIALOG_PARAM_REDIRECT_URI,
					REDIRECT_URI);
			parameters.putString(ServerProtocol.DIALOG_PARAM_CLIENT_ID,
					getApplicationId());
			parameters.putString(ServerProtocol.DIALOG_PARAM_E2E, e2e);
			parameters.putString(ServerProtocol.DIALOG_PARAM_RESPONSE_TYPE,
					ServerProtocol.DIALOG_RESPONSE_TYPE_TOKEN);
			parameters.putString(ServerProtocol.DIALOG_PARAM_RETURN_SCOPES,
			...
			if (isRerequest && !Settings.getPlatformCompatibilityEnabled()) {
				parameters.putString(ServerProtocol.DIALOG_PARAM_AUTH_TYPE,
						ServerProtocol.DIALOG_REREQUEST_AUTH_TYPE);
			}

			return new WebDialog(getContext(), OAUTH_DIALOG, parameters,
					getTheme(), getListener());
		}
	}
	...
	static class AuthorizationRequest implements Serializable {
		...
		AuthorizationRequest(SessionLoginBehavior loginBehavior,
				int requestCode, boolean isLegacy, List<String> permissions,
				SessionDefaultAudience defaultAudience, String applicationId,
				String validateSameFbidAsToken,
				StartActivityDelegate startActivityDelegate, String authId) {
	...
	static class Result implements Serializable {
		...
		enum Code {
			...
			// For consistency across platforms, we want to use specific string
			// values when logging these results.
		...
		private Result(AuthorizationRequest request, Code code,
				AccessToken token, String errorMessage, String errorCode) {
		...
		static Result createTokenResult(AuthorizationRequest request,
				AccessToken token) {
			return new Result(request, Code.SUCCESS, token, null, null);
		}

		static Result createCancelResult(AuthorizationRequest request,
				String message) {
			return new Result(request, Code.CANCEL, null, message, null);
		}

		static Result createErrorResult(AuthorizationRequest request,
				String errorType, String errorDescription) {
			return createErrorResult(request, errorType, errorDescription, null);
		}

		static Result createErrorResult(AuthorizationRequest request,
				String errorType, String errorDescription, String errorCode) {
