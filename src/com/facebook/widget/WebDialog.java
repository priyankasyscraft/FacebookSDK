...
public class WebDialog extends Dialog {
	 ...
	 */
	public WebDialog(Context context, String action, Bundle parameters,
			int theme, OnCompleteListener listener) {
		super(context, theme);
		...
		// our webview client only handles the redirect uri we specify, so just
		// hard code it here
	...
	public void dismiss() {
		if (isDismissed) {
			// Some paths may cause dismiss() to be called recursively. Break
			// the loop here.
			return;
		}
		isDismissed = true;

		// If dismiss() was called without sending a result to the listener,
		// let's default to a "cancel" result.
		// This will be the case when the user taps the OS-back-button, or the
	...
	private void setUpWebView(int margin) {
		...
		webView = new WebView(getContext()) {
			/*
			 * Prevent NPE on Motorola 2.2 devices See
			 * https://groups.google.com/
			 * forum/?fromgroups=#!topic/android-developers/ktbwY2gtLKQ
			 */
			@Override
	...
	private static class BuilderBase<CONCRETE extends BuilderBase<?>> {
		...
		protected BuilderBase(Context context, String action) {
			...
			if (activeSession != null && activeSession.isOpened()) {
				...
				String applicationId = Utility
						.getMetadataApplicationId(context);
				if (applicationId != null) {
					this.applicationId = applicationId;
				} else {
					throw new FacebookException(
							"Attempted to create a builder without an open"
		...
		protected BuilderBase(Context context, Session session, String action,
				Bundle parameters) {
		...
		protected BuilderBase(Context context, String applicationId,
				String action, Bundle parameters) {
			if (applicationId == null) {
		...
		/**
		 * Sets a theme identifier which will be passed to the underlying
		 * Dialog.
		 * 
		 * @param theme
		 *            a theme identifier which will be passed to the Dialog
		 *            class
		 * @return the builder
		 */
		 ...
		 * 
		 * @param listener
		 *            the listener to notify, or null if no notification is
		 *            desired
		 * @return the builder
		 */
		...
		/**
		 * Constructs a WebDialog using the parameters provided. The dialog is
		 * not shown, but is ready to be shown by calling Dialog.show().
		 * 
		 * @return the WebDialog
		 */
		public WebDialog build() {
			if (session != null && session.isOpened()) {
				parameters.putString(ServerProtocol.DIALOG_PARAM_APP_ID,
		...
		private void finishInit(Context context, String action,
				Bundle parameters) {
	...
	/**
	 * Provides a builder that allows construction of an arbitrary Facebook web
	 * dialog.
	 */
	public static class Builder extends BuilderBase<Builder> {
		/**
		 * Constructor that builds a dialog using either the active session, or
		 * the application id specified in the application/meta-data.
		 * 
		 * @param context
		 *            the Context within which the dialog will be shown.
		 * @param action
		 *            the portion of the dialog URL following
		 *            www.facebook.com/dialog/. See
		 ...
		 */
		public Builder(Context context, Session session, String action,
				Bundle parameters) {
		 ...
		 *            the Context within which the dialog will be shown.
		 * @param applicationId
		 *            the application ID to be included in the dialog URL.
		 * @param action
		 *            the portion of the dialog URL following
		 *            www.facebook.com/dialog/. See
		 *            https://developers.facebook.com/docs/reference/dialogs/
		 ...
		 *            a Bundle containing parameters to pass as part of the URL.
		 */
		public Builder(Context context, String applicationId, String action,
				Bundle parameters) {
	 ...
	 * the <a
	 * href="https://developers.facebook.com/docs/reference/dialogs/feed">Feed
	 * Dialog</a>.
	 */
	public static class FeedDialogBuilder extends
		...
		/**
		 * Constructor that builds a Feed Dialog using either the active
		 * session, or the application ID specified in the
		...
		/**
		 * Constructor that builds a Feed Dialog using the provided session and
		 * parameters.
		 * 
		 * @param context
		 *            the Context within which the dialog will be shown.
		 ...
		 *            dialog URL. No validation is done on these parameters; it
		 *            is the caller's responsibility to ensure they are valid.
		 *            For more information, see <a
		 *            href="https://developers.facebook.com/docs/reference/dialogs/feed/"
		 *            >
		 ...
		 *            the Context within which the dialog will be shown.
		 * @param applicationId
		 *            the application ID to use. If null, the application ID
		 *            specified in the application/meta-data will be used
		 *            instead.
		 * @param parameters
		 *            a Bundle containing parameters to pass as part of the
		 *            dialog URL. No validation is done on these parameters; it
		 ...
		 *            /</a>.
		 */
		public FeedDialogBuilder(Context context, String applicationId,
				Bundle parameters) {
			super(context, applicationId, FEED_DIALOG, parameters);
		}

		/**
		 * Sets the ID of the profile that is posting to Facebook. If none is
		 * specified, the default is "me". This profile must be either the
		 * authenticated user or a Page that the user is an administrator of.
		 * 
		 * @param id
		...
		/**
		 * Sets the ID of the profile that the story will be published to. If
		 * not specified, it will default to the same profile that the story is
		 * being published from. The ID must be a friend who also uses your app.
		 * 
		 * @param id
		...
		/**
		 * Sets the URL of a media file attached to this post. If this is set,
		 * any picture set via setPicture will be ignored.
		 * 
		 * @param source
		...
		/**
		 * Sets the name of the URL being shared. This method only has effect if
		 * setLink is called.
		 * 
		 * @param name
		 *            the name
		...
		/**
		 * Sets the caption of the URL being shared. This method only has effect
		 * if setLink is called.
		 * 
		 * @param caption
		 *            the caption
		 * @return the builder
		...
		/**
		 * Sets the description of the URL being shared. This method only has
		 * effect if setLink is called.
		 * 
		 * @param description
		 *            the description
		 * @return the builder
	 ...
	 * Provides a builder that allows construction of the parameters for showing
	 * the <a
	 * href="https://developers.facebook.com/docs/reference/dialogs/requests"
	 * >Requests Dialog</a>.
	 */
	public static class RequestsDialogBuilder extends
		...
		/**
		 * Constructor that builds a Requests Dialog using either the active
		 * session, or the application ID specified in the
		 * application/meta-data.
		 * 
		 * @param context
		...
		/**
		 * Constructor that builds a Requests Dialog using the provided
		 ...
		 *            /</a>.
		 */
		public RequestsDialogBuilder(Context context, String applicationId,
				Bundle parameters) {
			super(context, applicationId, APPREQUESTS_DIALOG, parameters);
		}

		/**
		 * Sets the string users receiving the request will see. The maximum
		...
		/**
		 * Sets the user ID or user name the request will be sent to. If this is
		 * not specified, a friend selector will be displayed and the user can
		 * select up to 50 friends.
		 * 
		 * @param id
		...
		/**
		 * Sets optional data which can be used for tracking; maximum length is
		 * 255 characters.
		 * 
		 * @param data
		 *            the data
		...
		/**
		 * Sets an optional title for the dialog; maximum length is 50
		 * characters.
		 * 
		 * @param title
		 *            the title
